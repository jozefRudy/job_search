/**
 * Generated by orval v8.18.0 🍺
 * Do not edit manually.
 * jobsearch
 * OpenAPI spec version: 0.4.33
 */
export interface ApplyRequest {
  applied: boolean;
}

/**
 * Full detail scraped from an individual Upwork job page.
 */
export interface UpworkJobDetail {
  description: string;
  duration: string;
  exact_budget: string;
  experience_level: string;
  hires: string;
  hours_per_week: string;
  interviewing: string;
  invites_sent: string;
  /** @nullable */
  last_viewed?: string | null;
  posted_at: string;
  project_type: string;
  proposals: string;
  tags: string[];
  unanswered_invites: string;
}

/**
 * Full detail scraped from an individual `NoFluffJobs` job page.
 */
export interface NoFluffJobDetail {
  company: string;
  description: string;
  /** @nullable */
  employment_type?: string | null;
  languages: string[];
  locations: string[];
  must_have: string[];
  nice_to_have: string;
  offer_valid_until: string;
  posted_at: string;
  requirements: string;
  seniority: string;
}

/**
 * Full detail scraped from an individual eFinancialCareers job page.
 */
export interface EfinancialcareersJobDetail {
  company: string;
  description: string;
  employment_type: string;
  location: string;
  posted_at: string;
  salary: string;
  work_arrangement_type: string;
}

/**
 * Full detail scraped from an individual Hacker News "Who is hiring?" comment.
 */
export interface HackerNewsJobDetail {
  author: string;
  author_threads_url: string;
  /** @nullable */
  company?: string | null;
  description: string;
  /** @nullable */
  location?: string | null;
  /** @nullable */
  role?: string | null;
}

/**
 * Full detail scraped from a LinkedIn job posting via Voyager API.
 */
export interface LinkedInJobDetail {
  company: string;
  description: string;
  employment_type: string;
  industries: string;
  job_function: string;
  location: string;
  posted_at: string;
  salary: string;
  workplace_type: string;
}

/**
 * Full detail of a Work at a Startup (Y Combinator) job from the Algolia
 * `WaaSPublicCompanyJob` index.
 */
export interface WorkAtStartupJobDetail {
  /** @nullable */
  company_description?: string | null;
  company_name: string;
  /** @nullable */
  company_parent_sector?: string | null;
  /**
     * @minimum 0
     * @nullable
     */
  company_team_size?: number | null;
  /** @nullable */
  company_waas_stage?: string | null;
  /** @nullable */
  company_website?: string | null;
  description: string;
  eng_type: string[];
  has_equity: boolean;
  has_interview_process: boolean;
  has_salary: boolean;
  job_type: string;
  /**
     * @minimum 0
     * @nullable
     */
  min_experience?: number | null;
  remote: string;
  role: string;
  skills: string[];
  us_visa_required: string;
}

/**
 * Full detail from an individual Reddit post or megathread comment.
 * `is_job_ad` is an extraction-layer gate only and intentionally absent here.
 */
export interface RedditJobDetail {
  author: string;
  /** @nullable */
  company?: string | null;
  description: string;
  /** @nullable */
  location?: string | null;
  permalink: string;
  posted_at: string;
  /** @nullable */
  role?: string | null;
  subreddit: string;
}

/**
 * Full detail of a Wellfound job from the SSR Apollo cache
 * (`JobListingSearchResult` + resolved `StartupResult`).
 */
export interface WellfoundJobDetail {
  accepted_remote_location_names: string[];
  /** @nullable */
  company_high_concept?: string | null;
  /** @nullable */
  company_logo_url?: string | null;
  company_name: string;
  /** @nullable */
  company_size?: string | null;
  /** @nullable */
  company_slug?: string | null;
  /** @nullable */
  compensation?: string | null;
  description: string;
  job_type: string;
  location_names: string[];
  posted_at: string;
  /** @nullable */
  primary_role_title?: string | null;
  remote: boolean;
  /** @nullable */
  remote_kind?: string | null;
  /**
     * @minimum 0
     * @nullable
     */
  years_experience_max?: number | null;
  /**
     * @minimum 0
     * @nullable
     */
  years_experience_min?: number | null;
}

/**
 * Platform-specific scraped data stored on each job.
 */
export type Data = {
  detail: UpworkJobDetail;
  platform: 'upwork';
} | {
  detail: NoFluffJobDetail;
  platform: 'nofluffjobs';
} | {
  detail: EfinancialcareersJobDetail;
  platform: 'efinancialcareers';
} | {
  detail: HackerNewsJobDetail;
  platform: 'hackernews';
} | {
  detail: LinkedInJobDetail;
  platform: 'linkedin';
} | {
  detail: WorkAtStartupJobDetail;
  platform: 'workatastartup';
} | {
  detail: RedditJobDetail;
  platform: 'reddit';
} | {
  detail: WellfoundJobDetail;
  platform: 'wellfound';
};

export type Platform = typeof Platform[keyof typeof Platform];


export const Platform = {
  efinancialcareers: 'efinancialcareers',
  hackernews: 'hackernews',
  nofluffjobs: 'nofluffjobs',
  upwork: 'upwork',
  linkedin: 'linkedin',
  workatastartup: 'workatastartup',
  reddit: 'reddit',
  wellfound: 'wellfound',
} as const;

export type Rating = typeof Rating[keyof typeof Rating];


export const Rating = {
  liked: 'liked',
  disliked: 'disliked',
  neutral: 'neutral',
} as const;

export interface Job {
  /** @nullable */
  applied_at?: string | null;
  /** @nullable */
  budget?: string | null;
  /** @nullable */
  company?: string | null;
  created_at: string;
  external_id: string;
  id: number;
  /** @nullable */
  note?: string | null;
  platform: Platform;
  rating: Rating;
  raw: Data;
  remote: boolean;
  tags: string[];
  title: string;
  updated_at: string;
  url: string;
}

export interface JobListResponse {
  capped: boolean;
  jobs: Job[];
  /** @minimum 0 */
  total: number;
}

export interface RateRequest {
  rating: Rating;
}

/**
 *
 *  * Sorts available in the API and CLI.
 *  *
 *  * `Relevance` is used only by the vector search path and is not DB-sortable.
 */
export type Sort = typeof Sort[keyof typeof Sort];


export const Sort = {
  created: 'created',
  upwork_viewed: 'upwork_viewed',
  applied: 'applied',
  relevance: 'relevance',
} as const;

export type ListJobsParams = {
platform?: Platform;
rating?: Rating;
applied?: boolean;
remote?: boolean;
/**
 * Only jobs created within the last N days.
 * @minimum 0
 */
recency?: number;
search?: string;
sort_by?: Sort;
/**
 * @minimum 0
 */
page?: number;
/**
 * @minimum 0
 */
page_size?: number;
};

